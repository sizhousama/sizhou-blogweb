...
const cateItem: React.FC<childProps> = props => {
  ...
  const tags = (cat: any) => {
    return cat.tags.map((tag: any, index: number) => {
      return (
        <span
          className={`mr_10 mb_10 cp ${styles.cattag} ${
            Number(curTag) === tag.id ? styles.act : ''
          }`}
    ...
    <>
      {cat.tags.length > 0 ? (
        <Popover placement="bottom" content={tags(cat)} trigger="hover">
          <span
            className={`cp ${styles.cat} ${
              Number(curCat) === cat.id ? styles.act : ''
            }`}
...
export default connect(({ article }: { article: ArtState }) => ({
  article,
}))(cateItem);
